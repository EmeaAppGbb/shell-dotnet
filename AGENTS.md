...
**Stack:** .NET 10 (ASP.NET Core Minimal API) + Vue 3.5 (TypeScript, Vite) + Azure Cosmos DB

### Project Structure

```
src/
 backend/          # ASP.NET Core Minimal API (.NET 10, single-file Program.cs)
...
tests/
 agentic-api-tests/    # API tests
specs/                    # PRD, FRDs, Gherkin, UI prototypes, contracts
 docs/                 # Architecture, technology, infrastructure docs
...
| Target | Command |
|---|---|
| Backend | `cd src/backend && dotnet build` |
| Frontend | `cd src/frontend && npm run build` |
| Frontend lint | `cd src/frontend && npm run lint` |
| Frontend type check | `cd src/frontend && npm run type-check` |

### Test Commands

| Test Type | Command | Notes |
|---|---|---|
| Unit tests (Frontend) | `cd src/frontend && npm run test:unit` | Vitest |
| E2E tests (Frontend) | `cd src/frontend && npm run test:e2e` | Playwright |
| All frontend tests | `cd src/frontend && npm run lint && npm run test:unit && npm run test:e2e` | Full regression |
...
**Cosmos DB:**
- Point reads (ID + partition key) = 1 RU; avoid cross-partition queries
- Partition key change = delete + create (cannot update partition key)

### Environment Variables
- `AZURE_COSMOS_DB_ Cosmos endpoint (auto-set by Aspire)ENDPOINT` 
- `VITE_API_BASE_ Backend URL (empty = same origin)URL` 
...
### Gotchas
- Single-file backend (~325  split for larger projectslines) 
- No health  add `/health` for Container Appsendpoint 
- Cross-partition ` add pagination for scaleGetAllAsync()` 
