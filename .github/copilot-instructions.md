...
### .NET / ASP.NET Core Conventions ( src/backend/)Backend 

- **Minimal API**: Use `app.Map*()` endpoints in `Program.cs`. Always chain `.WithName()` and `.WithOpenApi()`
- **PascalCase**: PascalCase for types/methods/properties, camelCase for local variables and parameters
- **Records for DTOs**: Use `record` types for API request/response DTOs. Use classes with `[JsonPropertyName]` for Cosmos DB documents
- **Async/await**: All I/O-bound operations must be async. Use `async` lambdas in endpoint handlers
- **Result pattern**: Use `Results.Ok()`, `Results.NotFound()`, `Results. consistent response shapesBadRequest()` 
- **Dependency injection**: Register services in `Program.cs` via `builder.Services`. Use constructor injection
- **Cosmos DB**: Use Aspire's `builder.AddAzureCosmosClient()` for client injection. Point reads over queries where possible
- **Configuration**: Use `IConfiguration` and `appsettings.json`. Environment-specific overrides in `appsettings.{Environment}.json`
- **No console.log equivalent**: Use `ILogger<T>` for structured  no `Console.WriteLine` in productionlogging 
- **Nullable enabled**: `<Nullable>enable</ use null checks and nullable annotations consistentlyNullable>` 

### Vue 3 / TypeScript Conventions ( src/frontend/)Frontend 

- **Composition API only**: Use `<script setup lang=" no Options APIts">` 
- **TypeScript strict**: Use `interface` over `type` for object shapes. No `any` types
- **Pinia for state**: Use Pinia stores for shared state. Composables (`use*`) for reusable logic
- **Vue Router**: Routes in `src/router/index.ts`. Use named routes for navigation
- **Scoped styles**: Always use `<style  no global CSS leaksscoped>` 
- **Component naming**: PascalCase for components, kebab-case for directories
- **Reactive refs**: Use `ref()` for primitives, `reactive()` for objects. Always use `.value` in script
- **Error handling**: Every API call must have try/catch with user-visible error state
- **Vite**: Use Vite for dev server and builds. Environment variables prefixed with `VITE_`

### Test Framework Conventions

- **Vitest**: Use `describe()` for grouping, `it()` or `test()` for individual tests. Use `vi.mock()` for mocking
- **Playwright**: Use `test.describe()` for grouping. No hardcoded  use auto-waiting locatorswaits 
- **Test naming**: `it('should [behavior] when [condition]')` for Vitest
- **Test isolation**: Each test must be  no shared mutable state between testsindependent 

### Aspire Orchestration

- **Prefer Aspire** for all local development: `dotnet run --project apphost.cs`
- **Service registration**: Define services in `apphost. backend, frontend, docscs` 
- **Environment variables**: Aspire auto-configures service URLs and connection strings
- **Dashboard**: Available at http://localhost:15888 for observability
